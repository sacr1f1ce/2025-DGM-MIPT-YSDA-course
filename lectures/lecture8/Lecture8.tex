...
\begin{frame}{Recap of Previous Lecture}
    ...
    \begin{block}{Noise-Conditioned Score Network}
        \begin{itemize}
            ...
            \vspace{-0.3cm}
            \[
                \sum_{t=1}^\top {\color{violet}\sigma_t^2} \cdot \bbE_{\pd(\bx)} \bbE_{q(\bx_t | \bx)}\bigl\| \bs_{\btheta, \sigma_t}(\bx_t) - \nabla_{\bx_t} \log q(\bx_t | \bx) \bigr\|^2_2 \rightarrow \min_{\btheta}
            \]
            \vspace{-0.5cm}
...
\begin{frame}{Gaussian Diffusion Model as VAE}
    ...
    \begin{block}{Forward Diffusion}
        \begin{itemize}
            ...
            \vspace{-0.3cm}
            \[
                q(\bz | \bx) = q(\bx_1, \dots, \bx_T | \bx_0) = \prod_{t = 1}^\top q(\bx_t | \bx_{t - 1}).
            \]
...
\begin{frame}{ELBO for Gaussian Diffusion Model}
    ...
    \begin{block}{Derivation}
        ...
        \begin{align*}
            ...
            \nextonslide{& = \bbE_{q(\bx_{1:T} | \bx_0)} \log \frac{p(\bx_T) \prod_{t=1}^\top \pt(\bx_{t-1} | \bx_t) }{\prod_{t=1}^\top {\color{violet}q(\bx_t | \bx_{t-1})}}}
        \end{align*}
        \vspace{-0.3cm}
        \eqpause
        \begin{itemize}
...
\begin{frame}{ELBO for Gaussian Diffusion Model}
    ...
    \begin{block}{Derivation (continued)}
        ...
        {\small
        \begin{align*}
            \cL_{\bphi, \btheta}(\bx) & = \bbE_{q(\bx_{1:T} | \bx_0)} \log \frac{p(\bx_T) \prod_{t=1}^\top \pt(\bx_{t-1} | \bx_t) }{\prod_{t=1}^\top {\color{violet}q(\bx_t | \bx_{t-1})}}  \\ 
            \nextonslide{& = \bbE_{q(\bx_{1:T} | \bx_0)} \log \frac{p(\bx_T) \prod_{t=1}^\top \pt(\bx_{t-1} | \bx_t) }{\prod_{t=1}^\top q(\bx_t | \bx_{t-1}, {\color{olive}\bx_0})}  }\\ 
