...
\begin{frame}{Conical Gaussian Paths}
	...
	\begin{multline*}
		...
		= \\ = \bbE_{t \sim U[0, 1]} \bbE_{\bx_1 \sim \pd(\bx)} {\color{violet}\bbE_{\bx_t \sim p_t(\bx | \bx_1)}}\left\| {\color{olive}\frac{\bx_1 - \bx_t}{1-t}} - \bff_{\btheta}({\color{teal}\bx_t}, t) \right\|^2
...
\begin{frame}{Score-Based Generative Models through SDEs}
	...
	\]
	\[
		\textbf{VE (NCSN): } p(\bx_t | \bx_1) = \cN\left(\bx_1, \sigma^2_{1-t} \cdot \bI\right)
	\]
	\vspace{-0.5cm}
	\[
		\textbf{VP (DDPM): } p(\bx_t | \bx_1) = \cN\left(\alpha_{1-t} \bx_1, \left(1 - \alpha_{1-t}^2\right) \cdot \bI\right)
	\]
	\vspace{-0.5cm}
	\eqpause
...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item Pair conditioning yields the same procedure, but is more general (suitable for paired tasks).
		\vfill
