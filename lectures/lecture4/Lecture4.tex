...
\begin{frame}{Amortized Variational Inference}
	\begin{block}{E-step}
		...
		\]
		\vspace{-0.3cm} \\
		$q(\bz)$ approximates the true posterior distribution $p(\bz| \bx, \btheta^*)$, which is why it is called the \textbf{variational posterior}.
		\begin{itemize}
			\item {\color{violet}$p(\bz| \bx, \btheta^*)$ may be \textbf{intractable}};
			\item {\color{teal}$q(\bz)$ is unique for each data point $\bx$}.
		\end{itemize}
	\end{block}
	\begin{block}{Variational Bayes}
		We limit the family of all possible distributions $q(\bz)$ to a parametric class $q(\bz|\bx, \bphi)$  {\color{teal}conditioned on samples $\bx$}  {\color{violet}and parameterized by $\bphi$}.
		\begin{itemize}
			\item E-step
			\[
...
\begin{frame}{Variational EM-algorithm}
	...
	\begin{itemize}
		...
		\]
		\vspace{-0.3cm} \\
		where $\bphi$ are the parameters of the variational posterior distribution $q(\bz | \bx, \bphi)$.
		\item \textbf{M-step:}
		\[
			\btheta_k = \btheta_{k-1} + \left.\eta \cdot \nabla_{\btheta} \cL_{\bphi_k, \btheta}(\bx)\right|_{\btheta=\btheta_{k-1}},
		\]
		where $\btheta$ are the parameters of the generative distribution $p(\bx | \bz, \btheta)$.
	\end{itemize}
	All that remains is to obtain \textbf{unbiased} Monte Carlo estimates of the gradients: $\nabla_{\bphi} \cL_{\bphi, \btheta}(\bx)$ and $\nabla_{\btheta} \cL_{\bphi, \btheta}(\bx)$. 
\end{frame}
%=======
...
\begin{frame}{ELBO Gradients, (M-step, $\nabla_{\btheta} \cL_{\bphi, \btheta}(\bx)$)}
	...
	\begin{block}{Naive Monte-Carlo estimation}
		...
		\vspace{-0.5cm} 
	\end{block}
	The variational posterior $q(\bz|\bx, \bphi)$ usually concentrates more probability mass in a smaller region than the prior $p(\bz)$. 
...
\begin{frame}{ELBO Gradients, (E-step, $\nabla_{\bphi} \cL_{\bphi, \btheta}(\bx)$)}
	\begin{block}{E-step: $\nabla_{\bphi} \cL_{\bphi, \btheta}(\bx)$}
		Unlike in the M-step, the density $q(\bz| \bx, \bphi)$ now depends on $\bphi$, so standard Monte Carlo estimation cannot be applied:
		\begin{align*}
	...
	\begin{block}{Reparametrization trick (LOTUS trick)} 
		Suppose $\bz \sim q(\bz | \bx, \bphi)$ is generated by a random variable $\bepsilon \sim p(\bepsilon)$ through the deterministic mapping $\bz = \bg_{\bphi}(\bx, \bepsilon)$. Then
		\[
			\bbE_{\bz \sim q(\bz | \bx, \bphi)} \bff(\bz) = \bbE_{\bepsilon \sim p(\bepsilon)} \bff(\bg_{\bphi}(\bx, \bepsilon))
		\]
		\textbf{Note:} The LHS expectation is with respect to the parametric distribution $q(\bz | \bx, \bphi)$, while the RHS uses the non-parametric distribution $p(\bepsilon)$.
	\end{block}
\end{frame}
%=======
\begin{frame}{ELBO Gradients, (E-step, $\nabla_{\bphi} \cL_{\bphi, \btheta}(\bx)$)}
	...
	\begin{block}{Variational assumption} 
		...
		Thus, we can write $q(\bz| \bx, \bphi) = \text{NN}_e(\bx, \bphi)$, known as the \textbf{encoder}.
	\end{block}
\end{frame}
%=======
\begin{frame}{ELBO Gradient (E-step, $\nabla_{\bphi} \cL_{\bphi, \btheta}(\bx)$)}
	...
	\begin{block}{Reconstruction term}
		...
		\end{multline*}
		\vspace{-0.5cm} \\
		The generative distribution $p(\bx | \bz, \btheta)$ can be a neural network. \\
		We write $p(\bx | \bz, \btheta) = \text{NN}_d(\bz, \btheta)$, called the \textbf{decoder}.
	\end{block}
	\begin{block}{KL term}
		$p(\bz)$ is the prior distribution on the latent variables $\bz$, often taken as $p(\bz) = \cN (0, \bI)$.
		\[
			{\color{teal}\nabla_{\bphi} KL(q(\bz | \bx, \bphi) || p(\bz))} = \nabla_{\bphi} KL\left( \cN (\bmu_{\bphi}(\bx), \bsigma^2_{\bphi}(\bx)) || \cN (0, \bI) \right)
		\]
		This expression has a closed-form analytical solution.
	\end{block}
\end{frame}
%=======
...
\begin{frame}{Variational Autoencoder (VAE)}
	\begin{block}{Training (EM-algorithm)}
		\begin{itemize}
			...
			\item Compute the objective (applying the reparametrization trick):
			\vspace{-0.3cm}
			\[
			...
			\]
			\item Update parameters via gradient steps with respect to $\bphi$ and $\btheta$ using stochastic gradients (as in autograd).
		\end{itemize}
	\end{block}
	\begin{block}{Inference}
		...
		\end{itemize}
	\end{block}
	\textbf{Note:} The encoder $q(\bz | \bx, \bphi)$ is not required during generation.
\end{frame}
%=======
...
\begin{frame}{Variational Autoencoder}
	\begin{itemize}
		...
		\item The decoder $p(\bx | \bz, \btheta) = \text{NN}_d(\bz, \btheta)$ outputs parameters of the data distribution.
	\end{itemize}
	\begin{figure}[h]
		\centering
...
\begin{frame}{VAE vs Normalizing Flows}
	...
	\end{table}
	\begin{block}{Theorem}
		MLE for a normalizing flow is equivalent to maximizing the ELBO for a VAE model with a:
		\vspace{-0.3cm}
		\[
...
\begin{frame}{Discrete VAE Latents}
	\begin{block}{Motivation}
		\begin{itemize}
			\item Previous VAE models used \textbf{continuous} latent variables $\bz$.
			\item \textbf{Discrete} representations $\bz$ can be a more natural fit for certain modalities.
			\item Powerful autoregressive models (such as PixelCNN) are effective for modeling distributions over discrete variables.
			\item Modern transformer-like models process discrete tokens.
		\end{itemize}
	\end{block}
	\begin{block}{ELBO}
		\vspace{-0.3cm}
	...
	\end{block}
	\begin{itemize}
		\item Use the reparametrization trick to obtain unbiased gradients.
		\item Adopt normal distributions for $q(\bz | \bx, \bphi)$ and $p(\bz)$ to enable analytical computation of the KL term.
	\end{itemize}
\end{frame}
%=======
\begin{frame}{Discrete VAE Latents}
...
\begin{frame}{Summary}
	\begin{itemize}
		\item Amortized variational inference enables efficient for the ELBO via Monte Carlo estimation.
		\vfill
		\item The reparametrization trick yields unbiased gradients with respect to the variational posterior $q(\bz | \bx, \bphi)$.
		\vfill
		\item The VAE model is a latent variable model with two neural networks: a stochastic encoder $q(\bz | \bx, \bphi)$ and a stochastic decoder $p(\bx | \bz, \btheta)$.
		\vfill
		\item NF models can be viewed as VAE models with deterministic encoder and decoder.
		\vfill
		\item Discrete VAE representations constitute a natural form of latent variables.
	\end{itemize}
\end{frame}
%=======
\end{document}