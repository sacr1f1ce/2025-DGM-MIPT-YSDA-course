...
\begin{frame}{Forward Discrete Process}
	...
	\begin{block}{Discrete Diffusion Markov Chain}
		...
		\]
		where each $\bx_t \in \{0,1\}^K$ is a \textbf{one-hot vector} encoding the categorical state.
	\end{block}
	\eqpause
...
%=======
%=======
\begin{frame}{Posterior Distribution $q(x_{t-1} \mid x_t, x_0)$}
	\myfootnotewithlink{https://arxiv.org/pdf/2107.03006}{Austin J. et al., \textit{Structured denoising diffusion models in discrete state-spaces}, 2021.}
	\begin{itemize}
		\item To train the reverse model, we need the posterior distribution
			\[
				q(x_{t-1} \mid x_t, x_0).
			\]
		\item From Bayes’ rule (Eq.~3 in the paper):
			\[
				q(x_{t-1} \mid x_t, x_0)
				= 
				\frac{
			...
			\]
		\item All three terms are tractable:
			\[
			\begin{aligned}
				q(x_t \mid x_{t-1}) &= \Cat(Q_t[x_{t-1},:]),\\
				q(x_{t-1} \mid x_0) &= \Cat(Q_{1:(t-1)}[x_0,:]),\\
				q(x_t \mid x_0) &= \Cat(Q_{1:t}[x_0,:]).
			\end{aligned}
			\]
		\item This expression defines a categorical distribution over $x_{t-1}$,
		      enabling exact computation of the discrete ELBO.
	\end{itemize}
\end{frame}
%=======
\begin{frame}{Posterior Distribution $q(x_{t-1} \mid x_t, x_0)$}
	...
	\begin{block}{Exact posterior (Eq.~3 in the paper, TRANSPOSE)}
		\[
		q(x_{t-1} \mid x_t, x_0)
		=
		\Cat\!\left(
			...
			\; \bp =
			\frac{
				x_t \bQ_t^{\!\top} \odot x_0 \, \bar{\bQ}_{t-1}
			}{
				x_0 \, \bar{\bQ}_t \, x_t^{\!\top}
			}
		\right).
		\]
	\end{block}
	...
	\eqpause
	\begin{itemize}
		\item $\bQ_t$ — forward transition at step $t$.
		\item $\bar{\bQ}_t = \bQ_t \bQ_{t-1}\!\cdots\bQ_1$ — cumulative transition.
